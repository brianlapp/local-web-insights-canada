...
async function detectTechnologyStack(url) {
  try {
    ...
    const response = await axios.get(`https://api.madewith.api/v1/lookup?url=${encodeURIComponent(url)}`, {
      headers: {
...
async function scheduleAuditsForAllBusinesses() {
  ...
  for (const business of businesses) {
    await auditQueue.add('audit-website', {
      ...
      // Spread out job processing to avoid rate limits
      delay: Math.floor(Math.random() * 60000) // Random delay up to 1 minute
    });
  }
  
  console.log('All audit jobs scheduled');
}

// Re-audit websites on a schedule
async function scheduleRecurringAudits() {
  // Re-audit businesses with old audits (older than 30 days)
  const thirtyDaysAgo = new Date();
  thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
  
  const { data: businessesToReaudit, error } = await supabase
    .from('businesses')
    .select('id, website')
    .not('website', 'is', null)
    .lt('last_audit_date', thirtyDaysAgo.toISOString());
    
  if (error) {
    console.error('Error fetching businesses to re-audit:', error);
    return;
  }
  
  console.log(`Scheduling re-audits for ${businessesToReaudit.length} businesses`);
  
  for (const business of businessesToReaudit) {
    await auditQueue.add('audit-website', {
      businessId: business.id,
      url: business.website
    }, {
...
// API route to manually trigger an audit
async function triggerAudit(businessId) {
  // Get business data
  ...
  if (!business.website) {
    throw new Error('Business does not have a website URL');
  }
...
// Export functions for use in API routes and scheduled jobs
export {
  runLighthouseAudit,
  captureScreenshots,
  ...
  scheduleAuditsForAllBusinesses,
  scheduleRecurringAudits,
  triggerAudit,
  auditQueue
};
